...
public class MobileUtilsScript : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (_freeCam)
        {
            ...
            if (_camLerp)
            {
                _targetCamPos += new Vector3(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical"), 0f) * Time.unscaledDeltaTime * (Input.GetKey(KeyCode.LeftShift) ? 3f : 1f) * _freeCamSpeed;
                _cameraTransform.position = Vector3.Lerp(_cameraTransform.position, new Vector3(_targetCamPos.x, _targetCamPos.y, _cameraTransform.position.z), lerp_freecam_speed * Time.unscaledDeltaTime);
            }else
                _cameraTransform.position += new Vector3(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical"), 0f) * Time.unscaledDeltaTime * (Input.GetKey(KeyCode.LeftShift) ? 3f : 1f) * _freeCamSpeed;
        }

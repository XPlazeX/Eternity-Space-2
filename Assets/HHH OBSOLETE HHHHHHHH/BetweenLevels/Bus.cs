...
public class Bus : MonoBehaviour
{
    ...
    private IEnumerator PlayerCatch()
    {
        ...
        {
            player.Translate((transform.position - player.position) * _catchSpeed * ESTime.worldDeltaTime);
    ...
    private IEnumerator FlyAway(bool shaking = false)
    {
        ...
        {
            speedPower += ESTime.worldDeltaTime * _busAcceleration;

            _bus.position += (Vector3.up) * speedPower * ESTime.worldDeltaTime;
            if (shaking)
                CameraController.Shake(0.075f);
            
            animationTimer -= ESTime.worldDeltaTime;

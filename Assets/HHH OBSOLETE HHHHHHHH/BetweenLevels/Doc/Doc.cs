...
public class Doc : MonoBehaviour
{
    ...
    private IEnumerator PlayerCatch()
    {
        ...
        {
            _palmCatch.transform.position += ((player.position - _palmCatch.position) * _catchSpeed * ESTime.worldDeltaTime);
        ...
        {
            player.position += ((_targetCatch.position - player.position) * _catchSpeed * ESTime.worldDeltaTime);
    ...
    private IEnumerator PlayerUncatch()
    {
        ...
        {
            player.Translate((_targetUncatch.position - player.position) * _catchSpeed * ESTime.worldDeltaTime);
            _palmCatch.position = player.position;
        ...
        {
            _palmCatch.Translate((_targetCatch.position - _palmCatch.position) * _catchSpeed * ESTime.worldDeltaTime);
    ...
    private IEnumerator Entering()
    {
        ...
        {
            transform.position = Vector3.Lerp(transform.position, toPosition, _flySpeed * ESTime.worldDeltaTime);

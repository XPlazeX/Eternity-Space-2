...
public class BarrelAnimator : MonoBehaviour
{
    ...
    private void FixedUpdate() 
    {
        ...
        {
            _targetTransform.localPosition = Vector3.Lerp(_targetTransform.localPosition, _targetLocalPosition, _workSpeed * ESTime.worldFixedDeltaTime);
        } else if (_working)
        {
            _working = false;
        } else if (!_working && (_startLocalPosition - _targetTransform.localPosition).magnitude > _deadzone)
        {
            _targetTransform.localPosition = Vector3.Lerp(_targetTransform.localPosition, _startLocalPosition, _repairSpeed * ESTime.worldFixedDeltaTime);
        }
    }

...
public class WarpDriveFieldControllerByDelta : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        float deltaPerSecond = delta.magnitude / Mathf.Max(ESTime.worldDeltaTime, 0.0001f);
        ...
        _currentWarp = Mathf.MoveTowards(_currentWarp, targetWarp, speed * ESTime.worldDeltaTime);

        targetRenderer.GetPropertyBlock(_mpb);
        _mpb.SetFloat(warpAmountProperty, _currentWarp);
        targetRenderer.SetPropertyBlock(_mpb);
        transform.up = SceneStatics.FlatVector(Vector3.RotateTowards(transform.up, delta.normalized, 3f * ESTime.worldDeltaTime, 3f * ESTime.worldDeltaTime));
    }
}
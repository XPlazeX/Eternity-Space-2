...
public class PlayerTargetMovement : MonoBehaviour
{
    ...
    private void Update() {
        ...
        if (_fearTimer <= 0)
        {
            transform.position = Vector3.Lerp(transform.position, _targetTransform.position, _lerpSpeed * ESTime.worldDeltaTime);
        } 
        else
        {
            _fearTimer -= ESTime.worldDeltaTime;
            transform.position = Vector3.Lerp(transform.position, _fearPosition, _lerpSpeed * ESTime.worldDeltaTime * 1.5f);
        }
    }

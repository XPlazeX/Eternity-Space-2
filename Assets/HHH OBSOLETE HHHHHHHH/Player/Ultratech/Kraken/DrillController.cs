    ...
    private IEnumerator Work()
    {
        ...
        while (timer > 0)
        {
            if (_parentTransform != null)
                transform.position = Vector3.Lerp(transform.position, _parentTransform.position + (_parentTransform.rotation * _followOffset), _followSpeed * ESTime.worldDeltaTime);
